...
import src.safesearch.verl.utils.hdfs_io as hdfs_io
...
from src.safesearch.verl.utils.debug import log_gpu_memory_usage
from src.safesearch.verl.utils.fs import copy_local_path_from_hdfs
from src.safesearch.verl.utils.fsdp_utils import get_fsdp_wrap_policy, offload_fsdp_grad, init_fn, get_init_weight_context_manager
from src.safesearch.verl.utils.fsdp_utils import offload_fsdp_optimizer, offload_fsdp_param_and_grad, load_fsdp_optimizer, \
    load_fsdp_param_and_grad
from src.safesearch.verl.utils.import_utils import import_external_libs
from src.safesearch.verl.utils.model import compute_position_id_with_mask
from src.safesearch.verl.utils.flops_counter import FlopsCounter
from src.safesearch.verl.workers.sharding_manager.fsdp_ulysses import FSDPUlyssesShardingManager
...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        from src.safesearch.verl.utils.model import print_model_size, update_model_config
        from src.safesearch.verl.utils.torch_dtypes import PrecisionType
        ...
        if use_remove_padding:
            from src.safesearch.verl.models.registry import check_model_support_rmpad
            check_model_support_rmpad(actor_model_config.model_type)

        if use_remove_padding and self.ulysses_sequence_parallel_size > 1:
            from src.safesearch.verl.models.transformers.monkey_patch import apply_monkey_patch
        ...
        if self._is_actor:
            from src.safesearch.verl.utils.torch_functional import get_constant_schedule_with_warmup
    ...
    def _build_rollout(self):
        ...
        if self.config.rollout.name == 'hf':
            from src.safesearch.verl.workers.rollout import HFRollout
            from src.safesearch.verl.workers.sharding_manager import BaseShardingManager
            rollout = HFRollout(module=self.actor_module_fsdp, config=self.config.rollout)
            rollout_sharding_manager = BaseShardingManager()
            # TODO: a sharding manager that do nothing?
        elif self.config.rollout.name == 'vllm':
            from src.safesearch.verl.workers.rollout.vllm_rollout import vLLMRollout
            from src.safesearch.verl.workers.sharding_manager import FSDPVLLMShardingManager
    ...
    def init_model(self):
        from src.safesearch.verl.workers.actor import DataParallelPPOActor
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name='update_policy', logger=None) as timer:
                metrics = self.actor.update_policy(data, self.tokenizer)
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        from src.safesearch.verl.utils.model import LambdaLayer, print_model_size, squeeze
        from src.safesearch.verl.utils.torch_dtypes import PrecisionType
        from torch.distributed.fsdp import FullyShardedDataParallel as FSDP, ShardingStrategy, MixedPrecision
        from torch import optim
    ...
    def init_model(self):
        ...
        from src.safesearch.verl.workers.critic import DataParallelPPOCritic
